...
### ⚠️ `retcode` is null on LAB/EXP — a regression in MVS/CE v3.0.0

Measured 2026-09-04. A job submitted to `MVSCE-LAB` reaches `status: OUTPUT` but
`retcode` stays `null` — **0 of 14 jobs** on the whole system have one.

The same read-only test against `MVSCE-DEV` returns **430 of 576**
(`MBTDEPL JOB02605 OUTPUT retcode='CC 0000'`). There the ones without are STCs
and TSU sessions, which carry no `NOTIFY` — the documented behaviour.

**And the older release is the one that works:**

| | `MVSCE-DEV` | `MVSCE-LAB` |
|---|---|---|
| `SYS1.PARMLIB(RELEASE)` | **v2.1.4** (2026-07-08) | **v3.0.0** (2026-08-01) |
| retcode | 430 of 576 | 0 of 14 |
| `zosmf_version` | `"1"`, with a `plugins` field | `"1.0.0-dev"`, without |

Both were unpacked from a release tarball the same way, so this is not a
difference in how they were installed. **Something between 2.1.4 and 3.0.0
...
spool output instead.

- [ ] Report the regression to MVS-sysgen: v3.0.0 lost what v2.1.4 has
- [ ] Meanwhile, consider running LAB/EXP on 2.1.4 like DEV
