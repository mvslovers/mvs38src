...
- **Not the job card, and not the missing auto-injection.** The mvsMF level in
  MVS/CE does not yet add `NOTIFY` by itself — but the test job carried one
  explicitly, and `JESJCL` confirms JES2 saw it:

  ```
  //TSTBR15  JOB (ACCT),CLASS=A,MSGCLASS=H,NOTIFY=IBMUSER,
  //         USER=IBMUSER,PASSWORD=   GENERATED BY MVSMF
  ```

  mvsMF appended only `USER=`/`PASSWORD=`. `retcode` stayed null regardless.
