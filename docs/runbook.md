...
### The job card — get this right or nothing works

✅ *verified 2026-09-04 on `MVSCE-LAB`*

```jcl
//JOBNAME   JOB (ACCT),'MVS38SRC',CLASS=A,MSGCLASS=H,NOTIFY=IBMUSER
//STEP1     EXEC PGM=IEFBR14
```

| Field | Rule |
|---|---|
| accounting `(ACCT)` | first positional |
| **programmer name** | **required.** Omit it and the job dies with `IEF633I PROGRAMMER NAME MISSING ON THE JOB STATEMENT` before it runs |
| `MSGCLASS=H` | a held class, so the output survives for inspection. Class `A` prints and purges, and the job is then gone from the API |
| `NOTIFY=` | may be set; the mvsMF level in MVS/CE does not add it automatically |
| `USER=` / `PASSWORD=` | **do not set them.** mvsMF appends them on a continuation card. Supplying your own `USER=` gives `IEF652I MUTUALLY EXCLUSIVE KEYWORDS` |

`PASSWORD=` shows as empty in `JESJCL` — that is JES2 masking it in the listing,
not an empty value.
...
on the evidence that `MVSCE-DEV` (v2.1.4) returned 430 retcodes out of 576 jobs
while `MVSCE-LAB` (v3.0.0) returned none out of 14, and that jobs on LAB ended
`$HASP396 TERMINATED` instead of `$HASP395 ENDED`.

**All of that was wrong.** Every job in the LAB sample was one of ours, and every
one had a job card missing the programmer name. They failed with a JCL error
before running, which is why they never reached normal termination and never got
a completion code. DEV's sample was the user's own jobs, with correct cards.

Two things had to be true at once, which is what made it convincing: the job
cards were broken **and** the mvsMF level then installed could not display
`JCL ERROR`, reporting `retcode: null` instead. With mvsMF updated, those same
old jobs now read `JCL ERROR` — the evidence was there all along, just not
displayable.

**The lesson for anyone measuring across two systems:** the samples were not
comparable. Before concluding anything from a DEV/LAB difference, run *the same
known-good job* on both. One control job would have saved a day.
