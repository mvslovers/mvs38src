...
**Corrected: it was one instance, not two.** I wrote that #339 did the same an
hour later with 26 decks on its `FURTHER` line. It did not. Checked rather than
recalled:

```
#335  base fb621ad (#330)   contains #337: NO   -- stale
#339  base b67af3d (#337)   contains #337: yes  -- not stale
```

#339's 26 were **real**, and cc370 had verified them module by module before
opening it: all 26 are rejected by both assemblers at `rc 12`, the EREP family,
and the PR body said so. I saw a familiar shape on the second PR and assumed the
first explanation, which is exactly the move this document keeps warning about —
**a second case that resembles the first is a case to measure, not a pattern.**
...
**And cc370's sharper statement of it**, which is the one to keep:

> A branch's gate is valid only against the base it was cut from, and that
> validity expires the moment `main` moves.

Not a stacked-PR trap — an **ageing** one. Their gate for #335 was internally
consistent against `main` as it then was, and useless to anyone gating an hour
later. Naming the baseline *directory* all week without naming what that
directory corresponds to on `main` is what made it invisible from both ends.
