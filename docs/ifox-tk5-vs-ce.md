...
## Operational: `MVSCE-EXP` cannot run jobs

Every job submitted to `MVSCE-EXP` (`:8083`) ends `JCL ERROR` /
`IEF722I ... INVALID PASSWORD GIVEN`. Its mvsMF generates the credential card
...
Reading works — `IBMUSER.PVTMAC` was copied off it member by member without
trouble — only submitting does not. **`tools/ifox_run.py` is hardcoded to
`:8083`** and would fail on its first job, which matters the moment stage 1's
corpus run is attempted. `MVSCE-LAB` (`:8082`) and both TK5 systems submit
normally.
