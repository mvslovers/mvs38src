...
alone. Last rewritten **2026-09-12**. Everything below the History heading is
dated and keeps its own numbers.*

### The goal, so it does not get lost again

**MVS 3.8j at maintenance level 8505, built from source.** One working question,
module by module: **which sources do not yet assemble to the object in TK5's
distribution libraries?** `cmplmd370` exits 0 or it does not. Full statement at
the head of [`README.md`](README.md), where the **scoreboard** also lives —
generated by `tools/scoreboard.py`, never hand-edited. `8505` is Mike's stated
target and is **not yet measured** against what TK5 actually carries.

New here? [`docs/README.md`](docs/README.md) maps the 47 documents and says which
are current.
...
| | |
|---|---:|
| **build's CSECT == IBM's shipped CSECT** | **1,422 of 5,485 — 25.9 %** |
| the same from the host, against TK5's DLIBs | 1,089 of 3,988 — 27.3 % |
| `as370` == IFOX00 | 5,427 of 5,528 at cc370 `fd287d3` — ⚠️ **distance 13**, so behind |
| **TK5's IFOX00 == MVS/CE's IFOX00** | **5,525 of 5,528 — stage 1 answered** |
| Dave's build on TK5 | runs 6 and 7 complete; his chain exhausted to `MAINT15G` |

```sh
git -C ~/repos/mvs/cc370 rev-list --count fd287d3..main    # before quoting row 3
```

### The Fahrplan, and only one stage is left

| | stage | state |
|---|---|---|
| 1 | is TK5's IFOX00 the same assembler? | ✅ **yes** — 5,525 of 5,528, 0 differing RLD cards |
| 2 | Dave's build on `MVSTK5-BLD` | ✅ runs 5–7 |
| 3 | `ZLMDRPTD`/`ZLMDRPTT` against TK5 | ✅ on the clean run-6 build |
| 4 | re-cut the reference against TK5 | ✅ **not needed** — the assemblers agree, so only the right-hand side moves and the recorded 5,528 decks stand |
| 5 | **the maintenance that never reached a source** | **the work. Not begun** |

### What 2026-09-11/12 established, shortest form

- **Dave's later-phase repairs reach IBM's object.** Three source states on one
  instrument: archive 1,084, applied-phase-1 1,076, **applied-phases-3-5 1,089**
  — the last is **+13 with an empty lost column** and beats the archive on
  distance too. [`docs/source-states.md`](docs/source-states.md).
- **His chain is exhausted.** 82 jobs past his own stop at `MAINT05F`, ending at
  `MAINT15G`. 54 ended non-zero for **one** reason: SMP refuses `./ DELETE`,
  proved by a two-sided case (`DSKTSTA` received, `DSKTSTB` — same file plus one
  `./ DELETE` — not). That costs 485 SYSMODs and leaves **486 modules without
  their `DSK` markers**. [`docs/dave-install-log.md`](docs/dave-install-log.md).
- **The `DS`-hole class is not benign, and that reverses a standing reading.**
  495 modules differ only in holes; the hole bytes are **identical between TK5
  and MVS/CE in 309 of 309** sampled clusters, which residue could not be, and
  they carry structure (`0ATSOMAC`). IBM initialises them, we do not. So
  **25.9 % is right as it stands** and 39.7 % would have been wrong to publish.
  [`docs/ds-holes.md`](docs/ds-holes.md).
- **And the holes are not the tool:** all 495 are modules where `as370`'s deck
  equals IFOX00's, so IFOX00 emits nothing there either.
- **`LPALIB` was never reached, not failed.** 1,916 of its 2,061 CSECTs with a
...
majority recovered — so the difference is history, not difficulty. Maintenance
arrives per library, which is the axis that can be attacked with a SYSMOD.

**2. The 147 cheapest in `LPALIB`** — same length, at most 4 bytes differing.
All 147 are real work again now that the hole class is refuted:
[`work/measurements/lpalib-smallest.tsv`](work/measurements/lpalib-smallest.tsv).

**3. Anchor `8505`.** The goal names a maintenance level nobody has measured.
The route is machine-readable: each load module's IDR carries a per-module
identifier, PTF numbers and IBM `RSI` stamps
([`docs/dlib-distance.md`](docs/dlib-distance.md)). Reading it across TK5's
DLIBs turns the goal from stated into measured.

### Waiting, and none of it blocks host-side work

- **Dave Kreiss — mail sent 2026-09-12.** `./ DELETE`, five macro items (largest
  `TABLE`, 45 measurable `XTB*` modules), and his rebuilt `PVTMAC`/`APVTMAC`.
- **cc370** has `IGARPT01` and is otherwise on its own ticket backlog.
- **mainframed767** on an MVS/CE 3.0.1 — sent 2026-09-06.

### Open for Mike

Nothing is currently blocked on him. The `DS`-hole reporting question that was
being prepared **dissolved**: the class turned out to be real defects, so there
is no reporting choice to make.
...
- **Both sides must see the same macro libraries.** On `MVSTK5-REF` that means
  the carried-across `IBMUSER.*` copies, not TK5's own `SYS1.A*` — the delta
  moves 68 modules.
- **Pin `ASMDATE`/`ASMTIME` and drop the `END` card** before comparing two runs.
  726 of 5,528 decks differ by the clock alone.
- **`grep` here is `ugrep` and `ls` is `eza`.** `ls -1 > list.txt` wrote eza's
  *header line* into a module list on 2026-09-12 and the first REST call
  answered HTTP 400. Use `/bin/ls`, `/usr/bin/grep`, or Python.
- **In zsh, a variable is not word-split.** `kill -TERM $PIDS` with newlines in
  it kills nothing and reports success. Use `xargs`.
- **`git add <missing-path>` stages a DELETE**, and a newline-separated command
  after a failed one still runs. One commit on 2026-09-12 removed a file its own
  message claimed to have edited. Chain with `&&`.
- **A scan with no hits is a claim about the instrument** until a control says
  otherwise.
- **Pin the binary by hash, not by path.** The default `as370` path points into a
  tree another session rebuilds continuously.
