...
## Seven from Dave Kreiss' own install tape — the ones nobody had

`BLDMVS.AWS`, his install package, holds `NEW.ASM` as a separate data set: 848
members, of which **321 are not in `MVSBLD` at all**. We had been measuring the
5,528 of `MVSBLD` and never looked at the tape's own source library.

Among the 321 are `HASPBLKS` and `HASPFMT0` through `FMT5` — the seven `AOSH3`
objects the `mvssrc` sessions had just reported as having **no source anywhere in
the archive**, presumed to be generated by a `HASPGEN` run nobody has.

All seven assemble byte-identical to IBM's shipped object on the first attempt,
`rc 0`. They are here.

The other 314 are kept unmeasured in `work/kreiss-newasm/`: 130 are macros we
already have, 20 are on the IBM tapes, and 48 are `XTB*` translate tables blocked
on a `TABLE`/`NAME` macro pair that exists in none of our libraries, none of the
tapes and not in `NEW.ASM` itself.
